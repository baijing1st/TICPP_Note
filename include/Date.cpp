...
Date::Date(int yr, int mon, int dy) throw(Date::DateError)
{
	if (!(1<=mon && mon<12))
	{
		throw Date::DateError("Bad month in Date ctor");
	}
	if ( !(1<= dy && dy<=daysInMonth[isleap(year)][mon]) )
	{
		throw Date::DateError("Bad day in Date ctor");
	}
	year = yr;
	month = mon;
	day = dy;
...
Date::Date(const std::string& s) throw(Date::DateError)
{
	...
	if (!(s.size() == 8))
	{
		throw Date::DateError("Bad string in Date ctor");
	}
	for (int n = 8; --n>=0;)
	{
		if (!isdigit(s[n]))
...
Date::Duration duration(const Date& date1, const Date& date2)
{
	int y1 = date1.year;
	int y2 = date2.year;
	int m1 = date1.month;
	...
	int months = m2 - m1;
	int days = d2 - d1;
	assert(years > 0 || years == 0 && months > 0 || months == 0 && days > 0 || days == 0);
	int lastMonth = m2;
	int lastYear = y2;
	while (days<0)
	{
...
istream& operator >> (std::istream& is, Date& d)
{
	is >> d.month;
	char dash;
	is >> dash;
